...
- [ADR-0009: Project-shape profile contract — location, precedence, and one-repo→N-entries](adr-0009-project-shape-profile-contract.md) — A single versioned `project-profile-v1` contract, readable inline in `gauge.config.json` (pull, config-inline wins) or from a source-owned `gauge.profile.json` (spec-006 push seam); one repo yields N portfolio entries via an optional `entries[]` list with composite `<baseId>-<entryId>` ids and a shared umbrella-repo git signal. Unblocks spec 007. (2026-08-03, Accepted)
